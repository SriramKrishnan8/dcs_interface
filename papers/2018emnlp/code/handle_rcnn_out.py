...
def main():
    ...
    input_file = open(rcnn_inp, 'r', encoding='utf-8')
    input_contents = input_file.read()
    input_file.close()
    
    terminal_sandhis_handled = handle_terminal_sandhis(input_contents)
    anusvara_handled = handle_anusvara(terminal_sandhis_handled)
    sa_handled = handle_sa(anusvara_handled)
    
    with open(new_rcnn_out, 'w') as f:
        f.write(sa_handled)
